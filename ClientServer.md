...
Вторая команда типа ExtensionRequest - **RPCFire.** Она вызывается в случае, если игрок нажал кнопку стрельбы и содержит информацию о локальном местоположении игрока (то есть то место, где, как думает клиент, он находится) и направлении его выстрела. Формат данных:
...
Со стороны сервера на клиент данные передаются с помощью команд двух типов: ExtensionRespons-ы и обновление данных MMO-сущностей. Опишем сначала формат данные ExtensionRespons-ов.
...
Каждая из MMO-сущностей содержит поле с ключом "sik", значение которого может быть 0 (для противников), 1 (для снарядов) и 2 (для башен). В зависимости от того, каково значение по тому ключу, MMO-сущность содержит разный набор данных.
...
Называние зоны (это одна из абстракций SmartFox сервера), к которой надо подсоединяться - `OpenWorldZone`. Для соединения с сервером выполняем следующие команды:
