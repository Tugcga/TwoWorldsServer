...
angle | Float | Угол направления в радианах, вычисляемый от положительного направления оси OX против часовой стрелки. Этот угол используется для запуска линейных снарядов, а posX и posY используются для снарядов, которые наносят урон только в конце своего пути
...
Вместо `sfs.LastJoinedRoom` можно во время события `OnRoomJoin` запомнить, к какой комнате присоединился клиент, и использовать её.